    ...
    <div class="bg-gradient-to-r from-sky-600 to-sky-700 px-6 py-4">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sky-100 mt-1">Ubah informasi kategori <?= esc($category['name']) ?></p>
            </div>
            ...
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-sm text-blue-700 mt-1">
                        Kategori ini digunakan oleh <strong><?= $category['location_count'] ?? 0 ?></strong> lokasi.
                        <?php if (($category['location_count'] ?? 0) > 0): ?>
                            Perubahan akan mempengaruhi semua lokasi yang menggunakan kategori ini.
                        <?php endif; ?>
                    </p>
                </div>
            </div>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <select id="icon" 
                                ...
                                class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent"
                                required>
                            <option value="fas fa-tint" <?= old('icon', $category['icon']) === 'fas fa-tint' ? 'selected' : '' ?>>💧 Tetesan Air</option>
                            <option value="fas fa-water" <?= old('icon', $category['icon']) === 'fas fa-water' ? 'selected' : '' ?>>🌊 Air</option>
                            <option value="fas fa-stream" <?= old('icon', $category['icon']) === 'fas fa-stream' ? 'selected' : '' ?>>🏞️ Aliran</option>
                            <option value="fas fa-seedling" <?= old('icon', $category['icon']) === 'fas fa-seedling' ? 'selected' : '' ?>>🌱 Tanaman</option>
                            <option value="fas fa-circle" <?= old('icon', $category['icon']) === 'fas fa-circle' ? 'selected' : '' ?>>⚫ Lingkaran</option>
                            <option value="fas fa-square" <?= old('icon', $category['icon']) === 'fas fa-square' ? 'selected' : '' ?>>⬛ Kotak</option>
                            <option value="fas fa-star" <?= old('icon', $category['icon']) === 'fas fa-star' ? 'selected' : '' ?>>⭐ Bintang</option>
                            <option value="fas fa-heart" <?= old('icon', $category['icon']) === 'fas fa-heart' ? 'selected' : '' ?>>❤️ Hati</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <div id="category-preview" class="inline-flex items-center space-x-3 bg-white border border-gray-200 rounded-lg p-3">
                    ...
                    <div>
                        ...
                        <div id="preview-status" class="text-xs px-2 py-1 rounded-full <?= old('status', $category['status']) === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' ?>">
                            <?= old('status', $category['status']) === 'active' ? 'Aktif' : 'Tidak Aktif' ?>
                        </div>
                    </div>
                </div>
            </div>
